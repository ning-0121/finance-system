        ...
        <Dialog open onOpenChange={() => setDialog(null)}>
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="grid grid-cols-2 gap-3 py-2 max-h-[60vh] overflow-y-auto">
              ...
              <div className="space-y-1"><Label className="text-xs">进项专票不含税额 *</Label><Input type="number" min={0} value={dialog.input_invoice_amount ?? 0} onChange={e => setDialog({ ...dialog, input_invoice_amount: Math.max(0, Number(e.target.value) || 0), refundable_amount: 0 })} /></div>
              <div className="space-y-1"><Label className="text-xs">退税率 %</Label><Input type="number" step="0.01" min={0} value={dialog.refund_rate ?? 13} onChange={e => setDialog({ ...dialog, refund_rate: Math.max(0, Number(e.target.value) || 0), refundable_amount: 0 })} /></div>
