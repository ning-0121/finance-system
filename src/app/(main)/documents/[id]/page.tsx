...
export default function DocumentConfirmPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const matches = (doc.extracted_fields?._matches || []) as { type: string; matched_name: string; detail: string; confidence: number }[]
                ...
                return (
                  <div key={f.field} className="space-y-1">
                    <Label className="text-xs flex items-center gap-1">
                      ...
                      {f.required && <span className="text-red-500">(必填)</span>}
                      {isChanged && <Badge variant="secondary" className="text-[9px] ml-1">已修改</Badge>}
                    </Label>
                    <Input
                      className={`h-8 text-sm ${isChanged ? 'border-amber-400 bg-amber-50' : ''}`}
                      value={String(value ?? '')}
