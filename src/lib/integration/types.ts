...
export type WebhookEventType =
  ...
  | 'shipment_approval.cancelled' // 出货申请撤回(业务撤回重报 → 队列未决行置 expired)
  ...
  | 'receivable.created'      // 应收创建(打样费等杂项应收:节拍器推 → 财务建独立应收 misc_receivables → 收款回传)
  | 'shipment.recorded'       // 出运档案(功能2):一票=一张提单;节拍器推 提单/CI 文件 + 关联订单(多对多)→ shipments
  | 'shipment.completed'      // 出货已完成【事实事件】(节拍器 2026-08-11 起发;≠审批请求)→ 落出运档案,财务跟应收
