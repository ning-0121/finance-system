...
import { allocateAmountByOrderQty } from '../src/lib/engines/cost-allocation'
...
// 权重来源改为 synced_orders.quantity 映射（budget_order_id → 数量）
...
section('按件数比例分摊 — 基础场景')
{
  ...
  const qty: QtyMap = { o1: 100, o2: 200, o3: 700 }
  const result = allocateAmountByOrderQty(10000, ['o1', 'o2', 'o3'], qty)
...
section('按件数比例分摊 — 含舍入差额')
{
  ...
  const qty: QtyMap = { o1: 1, o2: 1, o3: 1 }
  const result = allocateAmountByOrderQty(1000, ['o1', 'o2', 'o3'], qty)
  const sum = result.reduce((s, r) => s + r.amount, 0)
...
section('件数全为 0 → 平均分摊')
{
  const qty: QtyMap = { o1: 0, o2: 0, o3: 0 }
  const result = allocateAmountByOrderQty(900, ['o1', 'o2', 'o3'], qty)
...
{
  // 单订单 → 全部分摊到那个订单
  const qty: QtyMap = { o1: 100 }
  const single = allocateAmountByOrderQty(500, ['o1'], qty)
  ...
  // 零金额
  const zero = allocateAmountByOrderQty(0, ['o1'], qty)
  assert(zero.length === 0, '零金额 → 空数组')

  // 负金额（保护）
  const neg = allocateAmountByOrderQty(-100, ['o1'], qty)
...
{
  // 1 + 999 件 → 1% / 99%
  const qty: QtyMap = { o1: 1, o2: 999 }
  const r = allocateAmountByOrderQty(10000, ['o1', 'o2'], qty)
